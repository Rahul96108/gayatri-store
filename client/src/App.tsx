...
export default function Storefront() {
  ...
  const handleFinalCheckout = (addr: any) => {
    ...
    const msg = `*New Order Details*%0A%0A*Items:*%0A${items}%0A%0A*Total:* ₹${cartTotal}%0A%0A*Address:*%0A${addr.name}%0A${addr.phone}%0A${addr.line}%0A${addr.pin}`;
  ...
  return (
    <div className="min-h-screen bg-[#F5F1E6] text-[#2D1A12] font-sans selection:bg-[#D48C2B]">
      {/* GLOBAL CART OVERLAY (Always active layer) */}
        ...
        <div className="h-full flex flex-col p-8">
          ...
          {cart.length > 0 && (
            <div className="pt-8 border-t space-y-4">
              ...
              <button onClick={() => { setIsCartOpen(false); setView('checkout'); }} className="w-full bg-[#8B2312] text-white py-5 rounded-full font-black italic uppercase shadow-xl">Proceed to Checkout</button>
            </div>
          )}
        </div>
      </div>
