...
export default function Storefront() {
  ...
  const handleFinalCheckout = async (addr: any, orderId: string) => {
    ...
    if (error) {
      console.error(error);
      alert("System error saving order. Attempting WhatsApp instead.");
    }

    // Open WhatsApp
    const msg = `*Order ${orderId}*%0A%0A*Items:* ${itemDetails}%0A%0A*Total:* ₹${cartTotal}%0A%0A*Name:* ${addr.name}`;
