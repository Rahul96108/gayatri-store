...
// --- COMPONENT: EXPANDABLE CATALOG VIEW ---
    ...
    <div className="min-h-screen bg-[#F5F1E6] text-[#2D1A12] font-sans selection:bg-[#D48C2B]">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-40 pb-24 text-left">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
          <div className="space-y-4">
            <button onClick={onBack} className="text-[#8B2312] font-black italic uppercase text-xs flex items-center gap-2 group"><Home size={14}/> Back to Home</button>
            <h2 className="text-7xl md:text-8xl font-black italic uppercase text-[#8B2312] tracking-tighter leading-[0.85]">Full <br/> <span className="text-[#D48C2B]">Catalog</span></h2>
          </div>
          <div className="w-full md:w-96 space-y-2">
            <input type="text" placeholder="SEARCH THE PANTRY..." className="w-full bg-white border-2 border-[#2D1A12] p-5 rounded-2xl outline-none focus:border-[#D48C2B] font-bold uppercase text-xs tracking-widest shadow-sm" onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {filtered.map((p) => (
              <div key={p.id} className="group border-2 border-[#2D1A12] p-6 rounded-[3rem] bg-white hover:shadow-2xl transition-all flex flex-col">
                ...
                <button onClick={() => addToCart(p)} className="mt-auto bg-transparent border-2 border-[#2D1A12] py-4 rounded-full font-black italic uppercase text-[10px] flex items-center justify-center gap-2 hover:bg-[#8B2312] hover:text-white transition-all">Add to Cart <Plus size={14} /></button>
              </div>
            ))}
          </div>
        )}
    ...
    <div className="min-h-screen bg-[#F5F1E6] text-[#2D1A12] font-sans selection:bg-[#D48C2B]">
      ...
      <main ref={productsRef} className="max-w-7xl mx-auto px-6 py-24 text-left">
        ...
        {loading ? <div className="flex justify-center py-20"><Loader2 className="animate-spin text-[#8B2312] w-12 h-12" /></div> : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {products.map((p) => (
              <div key={p.id} className="border-2 border-[#2D1A12] p-8 rounded-[3.5rem] bg-white flex flex-col hover:shadow-xl transition-all">
        ...
        <div className="mt-20 flex flex-col items-center border-t border-[#2D1A12]/5 pt-20">
          <button onClick={() => { setView('catalog'); window.scrollTo(0,0); }} className="flex items-center gap-6 bg-[#2D1A12] text-white px-16 py-7 rounded-full font-black italic uppercase hover:bg-[#8B2312] transition-all shadow-2xl group active:scale-95">View Full Catalog <ArrowRight className="w-6 h-6 group-hover:translate-x-3 transition-transform" /></button>
        </div>
      </main>
      ...
      <footer className="bg-[#2D1A12] text-[#F5F1E6] pt-24 pb-12 px-8 md:px-16 border-t-[10px] border-[#D48C2B] text-left">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16">
          ...
          <div className="space-y-8">
            <h5 className="text-lg font-black italic uppercase text-white">Contact</h5>
            <div className="text-[10px] font-black uppercase opacity-50 space-y-4"><p>Gandhi Nagar, Bhilwara</p><p>+91 9982620643</p></div>
          </div>
        </div>
