      ...
      <div className={`fixed inset-y-0 right-0 w-full md:w-[400px] bg-white shadow-2xl z-[100] transform transition-transform duration-500 ease-in-out border-l-4 border-[#D48C2B] ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="h-full flex flex-col p-8">
          ...
          <div className="flex-grow overflow-y-auto space-y-6">
            {cart.length === 0 ? (
              <p className="text-center py-20 opacity-30 font-bold uppercase tracking-widest italic">Your bag is empty</p>
            ) : (
              cart.map(item => (
          ...
          {cart.length > 0 && (
            <div className="pt-8 border-t border-gray-100 space-y-4">
              ...
              <button onClick={checkoutViaWhatsApp} className="w-full bg-[#8B2312] text-white py-5 rounded-full font-black italic uppercase flex items-center justify-center gap-3 hover:bg-[#2D1A12] transition-all">
      ...
      <header className="relative min-h-[85vh] flex flex-col pt-24 px-8 md:px-16 overflow-hidden">
        <nav className="absolute top-0 left-0 w-full py-6 px-8 md:px-16 flex justify-between items-center z-20">
          <div className="flex items-center gap-3">
             <div className="w-10 h-10 rounded-full border border-[#8B2312]/20 flex items-center justify-center bg-white p-1 shadow-sm">
                <span className="text-[#8B2312] font-black text-[10px] uppercase tracking-tighter">Gaya</span>
             </div>
             <span className="font-black text-xl tracking-tighter text-[#8B2312] uppercase">GAYATRI</span>
          </div>
          <button onClick={() => setIsCartOpen(true)} className="bg-[#8B2312] text-white p-3 rounded-full shadow-lg hover:scale-110 transition-transform flex items-center gap-2">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between flex-grow gap-12">
          <div className="lg:w-1/2 space-y-8 text-center lg:text-left">
            ...
            <div className="space-y-2 text-[#2D1A12] opacity-70 font-bold uppercase tracking-widest text-sm">
              <p>Small batches. Pure groundnut oil.</p>
              <p>Authentic Rajasthani flavors.</p>
            </div>
            <button onClick={scrollToProducts} className="bg-[#8B2312] text-white px-10 py-5 rounded-full font-black italic uppercase text-lg shadow-xl hover:bg-[#2D1A12] transition-all transform active:scale-95">
              SHOP THE COLLECTION
            </button>
          </div>
          ...
          <div className="lg:w-1/2 flex justify-center lg:justify-end relative">
            <div className="w-72 h-72 md:w-[450px] md:h-[450px] rounded-full border-[15px] border-white shadow-2xl overflow-hidden bg-white/20 backdrop-blur-sm">
                <img src="https://images.unsplash.com/photo-1601050638917-3f04807b93dc?q=80&w=800&auto=format&fit=crop" alt="Product Display" className="w-full h-full object-cover opacity-90" />
            </div>
      ...
      <main ref={productsRef} className="max-w-7xl mx-auto px-6 py-24">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 text-left">
            {products.map((p) => (
              <div key={p.id} className="group border-2 border-[#2D1A12] p-8 rounded-[3.5rem] bg-white hover:shadow-2xl transition-all flex flex-col">
                <img src={p.image} className="w-full h-64 object-cover rounded-[2.5rem] mb-6 shadow-sm" alt={p.name} />
                ...
                <p className="text-xs opacity-50 font-bold uppercase tracking-widest mb-8 line-clamp-2">{p.description || "Traditional recipe, modern crunch."}</p>
                
                <button onClick={() => addToCart(p)} className="mt-auto bg-transparent border-2 border-[#2D1A12] text-[#2D1A12] py-4 rounded-full font-black italic uppercase flex items-center justify-center gap-3 hover:bg-[#8B2312] hover:border-[#8B2312] hover:text-white transition-all">
