...
import { 
  ...
  Phone, Mail, Instagram, Facebook, X, Plus, Minus, Send, Home 
} from 'lucide-react';
import './index.css';
...
// --- COMPONENT 1: DEDICATED CHECKOUT PAGE (Defined Outside) ---
    ...
    <div className="min-h-screen bg-[#F5F1E6] pt-32 pb-20 px-6 text-left">
      <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16">
        <div className="space-y-10">
          <button onClick={onBack} className="text-[#8B2312] font-black italic uppercase text-xs flex items-center gap-2">
            ← Back to Shop
          </button>
          <h2 className="text-5xl font-black italic uppercase text-[#8B2312] tracking-tighter leading-none">Shipping <br/> Details</h2>
          <div className="space-y-4 text-[#2D1A12]">
        ...
        <div className="bg-[#2D1A12] text-white p-10 rounded-[3.5rem] h-fit shadow-2xl">
          ...
          <button onClick={() => onComplete(address)} className="w-full bg-[#D48C2B] text-[#2D1A12] py-6 rounded-full font-black italic uppercase mt-10 hover:bg-white transition-all shadow-xl">
            Finalize Order
          </button>
        </div>
      </div>
...
// --- COMPONENT 2: DEDICATED CATALOG VIEW (Defined Outside) ---
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
          <div className="space-y-4">
            ...
            <h2 className="text-6xl font-black italic uppercase text-[#8B2312] tracking-tighter leading-none">The <br/> <span className="text-[#D48C2B]">Catalog</span></h2>
          </div>
          <input type="text" placeholder="SEARCH SNACKS..." className="w-full md:w-80 bg-white border-2 border-[#2D1A12] p-4 rounded-2xl outline-none focus:border-[#D48C2B]" onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {filtered.map((p) => (
              <div key={p.id} className="bg-white border-2 border-[#2D1A12] p-6 rounded-[2.5rem] flex flex-col hover:shadow-xl transition-all">
                ...
                <button onClick={() => addToCart(p)} className="mt-auto bg-[#8B2312] text-white py-3 rounded-full font-black italic uppercase text-[10px]">Add to Cart</button>
              </div>
            ))}
          </div>
        )}
...
export default function Storefront() {
  ...
  const handleFinalCheckout = (addr: any) => {
    ...
    const msg = `*Order Details*%0A%0A*Items:*%0A${items}%0A%0A*Total:* ₹${cartTotal}%0A%0A*Delivery To:*%0A${addr.name}%0A${addr.phone}%0A${addr.line}%0A${addr.pin}`;
        ...
        <div className="h-full flex flex-col p-8">
          ...
          {cart.length > 0 && (
            <div className="pt-8 border-t space-y-4">
              ...
              <button onClick={() => { setIsCartOpen(false); setView('checkout'); }} className="w-full bg-[#8B2312] text-white py-5 rounded-full font-black italic uppercase">Proceed to Checkout</button>
            </div>
          )}
        </div>
      </div>
      ...
      <header className="relative min-h-[85vh] flex flex-col pt-24 px-8 md:px-16 overflow-hidden text-left">
        <nav className="absolute top-0 left-0 w-full py-6 px-8 md:px-16 flex justify-between items-center z-20">
          <div className="flex items-center gap-3">
             <div className="w-10 h-10 rounded-full border border-[#8B2312]/20 flex items-center justify-center bg-white p-1 shadow-sm"><span className="text-[#8B2312] font-black text-[10px] uppercase tracking-tighter">Gaya</span></div>
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between flex-grow gap-12">
          <div className="lg:w-1/2 space-y-8 text-center lg:text-left">
            ...
            <button onClick={() => productsRef.current?.scrollIntoView({ behavior: 'smooth' })} className="bg-[#8B2312] text-white px-10 py-5 rounded-full font-black italic uppercase text-lg shadow-xl">SHOP THE COLLECTION</button>
          </div>
      ...
      <footer className="bg-[#2D1A12] text-[#F5F1E6] pt-24 pb-12 px-8 md:px-16 border-t-[10px] border-[#D48C2B] text-left">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16">
          <div className="space-y-8">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center p-2"><span className="text-[#8B2312] font-black text-xs uppercase">Gaya</span></div>
          ...
          <div className="space-y-8">
            ...
            <div className="flex gap-4">
              <Instagram className="cursor-pointer hover:text-[#D48C2B]" />
              <Facebook className="cursor-pointer hover:text-[#D48C2B]" />
            </div>
          </div>
        </div>
