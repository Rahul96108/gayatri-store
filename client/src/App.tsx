...
const CheckoutPage = ({ cart, cartTotal, onBack, onComplete }: any) => {
  ...
  const handleFinalize = async () => {
    if (!address.name || !address.phone) return alert("Please fill in your name and phone!");
    ...
    return (
      <div className="min-h-screen bg-[#F5F1E6] flex items-center justify-center p-6 text-left">
        <div className="bg-white border-4 border-[#8B2312] p-12 rounded-[3.5rem] max-w-2xl w-full shadow-2xl">
          ...
          <p className="text-[#D48C2B] font-black text-xl mb-8 uppercase">Order ID: {orderId}</p>
          <button onClick={onBack} className="w-full bg-[#8B2312] text-white py-5 rounded-full font-black italic uppercase">Back to Shop</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#F5F1E6] pt-32 pb-20 px-6 text-left">
      <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16">
        <div className="space-y-10">
          <button onClick={onBack} className="text-[#8B2312] font-black italic uppercase text-xs">← Back</button>
          <h2 className="text-5xl font-black italic uppercase text-[#8B2312] leading-none">Checkout</h2>
          <div className="space-y-4">
            ...
            <textarea placeholder="Address" rows={3} className="w-full bg-white border-2 border-[#2D1A12] p-5 rounded-2xl outline-none" onChange={e => setAddress({...address, line: e.target.value})} />
        ...
        <div className="bg-[#2D1A12] text-white p-10 rounded-[3.5rem] shadow-2xl h-fit">
          ...
          <button onClick={handleFinalize} disabled={isSubmitting} className="w-full bg-[#D48C2B] text-[#2D1A12] py-6 rounded-full font-black italic uppercase mt-10 shadow-xl disabled:opacity-50">
            {isSubmitting ? "Saving..." : "Finalize Order"}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-[#F5F1E6] pt-40 pb-24 px-6 text-left">
      ...
      <div className="max-w-7xl mx-auto">
        <h2 className="text-7xl font-black italic uppercase text-[#8B2312] mb-12">Catalog</h2>
        <input type="text" placeholder="Search..." className="w-full md:w-96 bg-white border-2 border-[#2D1A12] p-5 rounded-2xl outline-none mb-12" onChange={(e) => setSearchTerm(e.target.value)} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {filtered.map((p) => (
              <div key={p.id} className="bg-white border-2 border-[#2D1A12] p-6 rounded-[2.5rem] flex flex-col hover:shadow-xl transition-all">
                <img src={p.image} className="aspect-square w-full object-cover rounded-[2rem] mb-6" alt="" />
...
export default function Storefront() {
  ...
  const handleFinalCheckout = async (addr: any, orderId: string) => {
    ...
    if (error) {
       console.error(error);
       alert("Saving to database failed, but order is proceeding via WhatsApp...");
    }

    const msg = `*Order ${orderId}*%0A*Customer:* ${addr.name}%0A*Items:* ${itemDetails}%0A*Total:* ₹${cartTotal}`;
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                {products.map((p) => (
                  <div key={p.id} className="border-2 border-[#2D1A12] p-8 rounded-[3.5rem] bg-white flex flex-col hover:shadow-2xl transition-all">
                    ...
                    <button onClick={() => addToCart(p)} className="mt-auto border-2 border-[#2D1A12] py-4 rounded-full font-black italic uppercase hover:bg-[#8B2312] hover:text-white transition-all">Add to Cart</button>
                  </div>
                ))}
              </div>
            )}
            <div className="mt-20 flex flex-col items-center border-t border-[#2D1A12]/5 pt-20">
              <button onClick={() => setView('catalog')} className="flex items-center gap-6 bg-[#2D1A12] text-white px-16 py-7 rounded-full font-black italic uppercase hover:bg-[#8B2312] transition-all group">View Full Catalog <ArrowRight className="w-6 h-6 group-hover:translate-x-3 transition-transform" /></button>
            </div>
          </main>
          <footer className="bg-[#2D1A12] text-[#F5F1E6] pt-24 pb-12 px-8 md:px-16 border-t-[10px] border-[#D48C2B] text-left">
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16">
              <div className="space-y-8"><h4 className="text-3xl font-black italic uppercase text-[#D48C2B]">GAYATRI</h4><p className="text-sm opacity-60 italic text-left">Authentic namkeen since 1994.</p></div>
